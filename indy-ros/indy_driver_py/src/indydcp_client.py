...
# gwkim
    ...
    # wylee
    @tcp_command(CMD_GET_GRIPPER_DATA, 'int4dArr')
...
#         return (data, data_size)
    ...
    def set_gripper_mode(self, mode):
        self.set_endtool_do(0, mode)

    def set_gripper_cmd(self, cmd):
        self.set_endtool_do(1, cmd)

    def get_gripper_curdata(self):
        cdata = self.get_gripper_data()
        if (cdata[2] & 0x80 == 0):
            val = cdata[1] + (cdata[2] << 8)
        else:
            p_val = cdata[1] + (cdata[2] << 8)
            val = ((p_val^65535) + 1)*-1
        return val

    def get_gripper_posdata(self):
        pdata = self.get_gripper_data()
        val = pdata[0] + (pdata[1] << 8)
        return val
